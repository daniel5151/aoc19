...
///
/// There are five <span title="As you can see, I know exactly how rockets
/// work.">amplifiers connected in series</span>; each one receives an input
...
/// input), a program that should run on your [existing Intcode computer](5).
...
/// an input instruction to ask the amplifier for its current _phase setting_
/// (an integer from `0` to `4`). Each phase setting is used _exactly once_, but
...
/// which would mean setting amplifier `A` to phase setting `3`, amplifier `B`
/// to setting `1`, `C` to `2`, `D` to `4`, and `E` to `0`. Then, you could
...
/// talk you through rewiring the amplifiers into a _feedback loop_:
///
///           O-------O  O-------O  O-------O  O-------O  O-------O
...
/// output is connected to amplifier `B`'s input, and so on. _However,_ the
...
/// _many times_.
///
/// In feedback loop mode, the amplifiers need _totally different phase
/// settings_: integers from `5` to `9`, again each used exactly once. These
...
/// the process, a `0` signal is sent to amplifier `A`'s input _exactly once_.
///
...
/// amplifier `E` is sent to the thrusters. Your job is to _find the largest
/// output signal that can be sent to the thrusters_ using the new phase
/// settings and feedback loop arrangement.
///
/// Here are some example programs:
///
/// * Max thruster signal _`139629729`_ (from phase setting sequence
///   `9,8,7,6,5`):
///
