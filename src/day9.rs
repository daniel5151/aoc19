...
/// some checks to demonstrate it is a _complete Intcode computer_.
///
...
/// in _position mode_: the parameter is interpreted as a position. Like
...
/// address `0`. Instead, they count from a value called the _relative base_.
/// The _relative base_ starts at `0`.
///
...
/// `-7` refers to memory address `50 + -7 = _43_`.
///
/// The relative base is modified with the _relative base offset_ instruction:
///
/// * Opcode `9` _adjusts the relative base_ by the value of its only parameter.
...
/// opcode, output any opcodes (and the associated parameter modes) that seem to
/// be functioning incorrectly, and finally output a BOOST keycode.
///
...
/// output a single value, the BOOST keycode. _What BOOST keycode does it
/// produce?_
