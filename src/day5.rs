...
/// run on [your existing Intcode computer](2) after a few modifications:
///
/// _First_, you'll need to add _two new instructions_:
///
/// * Opcode `3` takes a single integer as _input_ and saves it to the address
...
///   by _the number of values in the instruction_ after the instruction
///   finishes. Because of the new instructions, this amount is no longer always
///   `4`.
...
/// the system to test by running an _input_ instruction - provide it `1`, the
...
/// then the TEST alarms start to go off. Since the air conditioner <span
/// title="Honestly, I'm not sure what you expected.">can't vent its heat
...
///
/// * Opcode `5` is _jump-if-true_: if the first parameter is _non-zero_, it
///   sets the instruction pointer to the value from the second parameter.
///   Otherwise, it does nothing.
/// * Opcode `6` is _jump-if-false_: if the first parameter _is zero_, it sets
///   the instruction pointer to the value from the second parameter. Otherwise,
///   it does nothing.
/// * Opcode `7` is _less than_: if the first parameter is _less than_ the
...
///
/// Like all instructions, these instructions need to support _parameter modes_
/// as described above.
///
/// Normally, after an instruction is finished, the instruction pointer
/// increases by the number of values in that instruction. _However_, if the
/// instruction modifies the instruction pointer, that value is used and the
/// instruction pointer is _not automatically increased_.
///
/// For example, here are several programs that take one input, compare it to
/// the value `8`, and then produce one output:
///
/// * `3,9,8,9,10,9,4,9,99,-1,8` - Using _position mode_, consider whether the
///   input is _equal to_ `8`; output `1` (if it is) or `0` (if it is not).
/// * `3,9,7,9,10,9,4,9,99,-1,8` - Using _position mode_, consider whether the
///   input is _less than_ `8`; output `1` (if it is) or `0` (if it is not).
/// * `3,3,1108,-1,8,3,4,3,99` - Using _immediate mode_, consider whether the
///   input is _equal to_ `8`; output `1` (if it is) or `0` (if it is not).
/// * `3,3,1107,-1,8,3,4,3,99` - Using _immediate mode_, consider whether the
///   input is _less than_ `8`; output `1` (if it is) or `0` (if it is not).
///
...
/// get the ID of the system to test, _provide it `5`_, the ID for the ship's
