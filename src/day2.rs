...
impl IntCode {
    pub fn new(init_mem: Vec<usize>) -> IntCode {
        IntCode {
    ...
    pub fn reset(&mut self, noun: usize, verb: usize) {
...
/// <span title="Looks like SOMEONE forgot to change the switch to 'more
/// magic'.">escaped</span>. "That computer ran _Intcode_ programs like the
...
/// (called position `0`). Here, you will find an _opcode_ - either `1`, `2`, or
...
/// (`3`). To handle this opcode, you first need to get the values at the input
/// positions: position `9` contains `30`, and position `10` contains `40`.
/// _Add_ these numbers together to get `70`. Then, store this value at the
/// output position; here, the output position (`3`) is _at_ position `3`, so it
...
/// ## --- Part Two ---
///
/// "Good, the new computer seems to be working correctly! _Keep it nearby_
...
/// the initial state for the computer's _memory_. When you run an Intcode
/// program, make sure to start by initializing memory to the program's values.
/// A position in memory is called an _address_ (for example, the first value in
...
/// instruction's _parameters_. For example, in the instruction `1,2,3,4`, `1`
...
/// at addresses `1` and `2`, just like before. In this program, the value
/// placed in address `1` is called the _noun_, and the value placed in address
/// `2` is called the _verb_. Each of the two input values will be between `0`
/// and `99`, inclusive.
///
...
/// _reset the computer's memory to the values in the program_ (your puzzle
